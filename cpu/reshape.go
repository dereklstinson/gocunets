...
//ShapeToBatchNCHW4D Takes a Volume and Segments it into Batches to the size h,w given. and rounds up by one.  Values not used in new tensor will be zero
func ShapeToBatchNCHW4D(values []float32, dims []int32, h, w int32) ([]float32, []int32, error) {
	if len(dims) != 4 {
...
//ShapeToBatchNHWC4D Takes a Volume and Segments it into Batches to the size h,w given. and rounds up by one.  Values not used in new tensor will be zero
func ShapeToBatchNHWC4D(values []float32, dims []int32, h, w int32) ([]float32, []int32, error) {
	if len(dims) != 4 {
		return nil, nil, errors.New("The Length of dims should equal 4")
	}
	if dims[0] != int32(1) {
