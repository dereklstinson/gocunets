...
func (c *Layer) WeightsFillSlice(handle *cudnn.Handler, input interface{}, length int) error {
	return c.w.FillSlice(handle, input)
	//	return c.w.T().Memer().FillSlice(input)

}

//DeltaWeightsFillSlice will fill the weights with values
func (c *Layer) DeltaWeightsFillSlice(handle *cudnn.Handler, input interface{}, length int) error {
	return c.w.FillSlice(handle, input)
