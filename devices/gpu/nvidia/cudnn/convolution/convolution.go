...
type Ops struct {
	...
	perfbackdataset bool
	perfbackfilt    BackFilterPerformance
	perfbackfiltset bool
...
func (c *Ops) String() string {
	...
	var pbfs string
	if !c.perfforwardset {
		pfs = "Perfomance Forward Not Set"
	} else {
		pfs = c.perfforward.String()
	}
	if !c.perfbackdataset {
		pbds = "Performance Backward Data Not Set"
	} else {
		pbds = c.perfbackdata.String()
	}
	if !c.perfbackfiltset {
		pbfs = "Performance Backward Filter Not Set"
	} else {
		pbfs = c.perfbackfilt.String()
	}

	return fmt.Sprintf("ConvolutionOps{\n%v\n%v\n%v\n%v\n%v\n}\n", c.op, pfs, pbds, pbfs, c.mtforward)
}
...
func StageOperation(mode gocudnn.ConvolutionMode, data gocudnn.DataType, mathtype gocudnn.MathType, group int32, pad, stride, dilation []int32) (conv *Ops, err error) {
	...
	conv.op, err = gocudnn.CreateConvolutionDescriptor()
	if err != nil {
		return nil, err
	}
