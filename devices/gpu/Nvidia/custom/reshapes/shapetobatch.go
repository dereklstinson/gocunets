...
func (o *Ops) GetB2SOutputProperties(handle *cudnn.Handler, x *tensor.Volume, window, stride []int32) (cudnn.TensorFormat, cudnn.DataType, []int32, error) {
	if len(window) != 2 {
	...
	return cudnn.TensorFormat(frmt), cudnn.DataType(dtype), dims, err

}

//GetS2BOutputProperties returns the properties of the output
func (o *Ops) GetS2BOutputProperties(handle *cudnn.Handler, x *tensor.Volume, window, stride []int32) (cudnn.TensorFormat, cudnn.DataType, []int32, error) {
	if len(window) != 2 {
		return 255, 255, nil, errors.New("window can only have 2 elements")
	}
...
func (o *Ops) GetS2BOutputPropertiesPLUS(handle *cudnn.Handler, x *tensor.Volume, window, stride []int32) (cudnn.TensorFormat, cudnn.DataType, []int32, []int32, error) {
	if len(window) != 2 {
	...
	return cudnn.TensorFormat(frmt), cudnn.DataType(dtype), dims, n1n2, err
